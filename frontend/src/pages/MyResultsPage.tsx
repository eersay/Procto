  ...
  return (
    <main className={`min-h-screen ${theme === 'light' ? 'bg-slate-50 text-slate-900' : 'bg-neutral-950 text-white'}`}>
      ...
      <div className="relative mx-auto w-full px-6 py-8 sm:px-8 lg:px-12 xl:px-20 xl:py-12">
        ...
        <div className="mt-4">
          <h1 className={`text-2xl font-bold ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>My Results</h1>
        ...
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={label}
              className={`rounded-2xl border backdrop-blur-sm p-5 ${theme === 'light' ? 'border-slate-300 bg-white' : 'border-neutral-800 bg-neutral-900/60'}`}>
        ...
        <section className="mt-8 pb-16">
          {results.length === 0 ? (
            <div className={`rounded-2xl border backdrop-blur-sm p-16 text-center ${theme === 'light' ? 'border-slate-300 bg-white/90' : 'border-neutral-800 bg-neutral-900/60'}`}>
              ...
              <h3 className={`font-semibold mb-2 ${theme === 'light' ? 'text-slate-900' : 'text-neutral-200'}`}>No results yet</h3>
              <p className={`text-sm ${theme === 'light' ? 'text-slate-500' : 'text-neutral-600'}`}>Complete exams to see your results here</p>
            </div>
          ) : (
            <div className="space-y-3">
              {results.map(exam => (
                <button key={exam.id} onClick={() => navigate(`/result/${exam.id}`)}
                  className={`group w-full rounded-2xl border backdrop-blur-sm p-5 text-left hover:border-emerald-500/40 hover:shadow-lg hover:shadow-emerald-500/10 transition-all duration-200 ${theme === 'light' ? 'border-slate-300 bg-white/90' : 'border-neutral-800 bg-neutral-900/60'}`}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <h3 className={`text-base font-semibold group-hover:text-emerald-400 transition-colors ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>
                          {exam.exam.title}
                        </h3>
                  ...
                  <div className={`mt-4 pt-3 border-t flex justify-end ${theme === 'light' ? 'border-slate-200' : 'border-neutral-800'}`}>
