...
export default function ExamBuilderPage() {
  ...
  const getStatus = (exam: Exam) => {
    if (!exam.isPublished) return { label: 'DRAFT', cls: theme === 'light' ? 'border-slate-300 bg-slate-100 text-slate-600' : 'border-slate-600 bg-slate-800 text-slate-400' };
    ...
    return { label: 'ENDED', cls: theme === 'light' ? 'border-slate-300 bg-slate-100 text-slate-600' : 'border-slate-600 bg-slate-800 text-slate-500' };
  };
    ...
    <main className={`min-h-screen ${theme === 'light' ? 'bg-gradient-to-br from-slate-50 via-white to-slate-50 text-slate-900' : 'bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-100'}`}>
      ...
      <div className="relative mx-auto w-full max-w-7xl px-6 py-8 sm:px-8">
        ...
        <header className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className={`text-[0.65rem] ${theme === 'light' ? 'text-slate-500' : 'text-slate-600'}`}>Exam Builder</p>
            </div>
          </div>
        ...
        <div className="flex items-end justify-between mb-8">
          <div>
            <h1 className={`text-2xl font-bold ${theme === 'light' ? 'text-slate-900' : 'text-slate-100'}`}>Exams</h1>
        ...
        {exams.length === 0 ? (
          <div className={`rounded-2xl border border-dashed p-16 text-center ${theme === 'light' ? 'border-slate-300 bg-white/80' : 'border-slate-700 bg-slate-900/40'}`}>
            ...
            <h3 className={`text-lg font-semibold mb-2 ${theme === 'light' ? 'text-slate-800' : 'text-slate-300'}`}>No exams yet</h3>
          ...
          <div className="grid md:grid-cols-2 gap-5">
            {exams.map((exam) => {
              ...
              return (
                <div key={exam.id} className={`rounded-2xl border backdrop-blur-sm p-6 transition-all group ${theme === 'light' ? 'border-slate-200 bg-white/90 hover:border-slate-300' : 'border-slate-700/60 bg-slate-900/60 hover:border-slate-600/60'}`}>
                  <div className="flex items-start justify-between mb-3">
                    <h3 className={`text-base font-bold group-hover:text-cyan-300 transition-colors leading-tight ${theme === 'light' ? 'text-slate-900' : 'text-slate-100'}`}>{exam.title}</h3>
                  ...
                  <div className="space-y-1.5 text-xs text-slate-500 mb-5">
                    <div className="flex items-center gap-2">
                      <span className={theme === 'light' ? 'text-slate-500' : 'text-slate-600'}>▶</span>
                  ...
                  <div className={`flex gap-2 pt-4 border-t ${theme === 'light' ? 'border-slate-200' : 'border-slate-800'}`}>
                    <button
                      onClick={() => { setSelectedExamId(exam.id); setShowAddQuestionsModal(true); }}
                      className={`flex-1 rounded-lg border py-2 text-xs font-semibold transition-all ${theme === 'light' ? 'border-slate-300 bg-slate-100 text-slate-700 hover:border-cyan-500/40 hover:text-cyan-600' : 'border-slate-700 bg-slate-800/60 text-slate-400 hover:border-cyan-500/40 hover:text-cyan-300'}`}
                    >
                      Add Questions
                    </button>
      ...
      {showCreateModal && (
        <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto backdrop-blur-sm ${theme === 'light' ? 'bg-slate-900/40' : 'bg-black/70'}`}>
          <div className={`relative w-full max-w-2xl my-8 rounded-2xl border shadow-2xl p-8 ${theme === 'light' ? 'border-slate-300 bg-white' : 'border-slate-700/60 bg-slate-900 shadow-black/60'}`}>
            <div className="flex items-center justify-between mb-6">
              <h2 className={`text-lg font-bold ${theme === 'light' ? 'text-slate-900' : 'text-slate-100'}`}>Create New Exam</h2>
            ...
            <form onSubmit={handleCreateExam} className="space-y-5">
              ...
              <div className={`rounded-xl border p-4 space-y-3 ${theme === 'light' ? 'border-slate-300 bg-slate-100/70' : 'border-slate-700/60 bg-slate-800/40'}`}>
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input type="checkbox" checked={formData.shuffleQuestions}
                    onChange={e => setFormData({ ...formData, shuffleQuestions: e.target.checked })}
                    className="w-4 h-4 accent-cyan-500" />
                  <span className={`text-sm transition-colors ${theme === 'light' ? 'text-slate-700 group-hover:text-slate-900' : 'text-slate-300 group-hover:text-slate-100'}`}>Shuffle question order for each student</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input type="checkbox" checked={formData.shuffleChoices}
                    onChange={e => setFormData({ ...formData, shuffleChoices: e.target.checked })}
                    className="w-4 h-4 accent-cyan-500" />
                  <span className={`text-sm transition-colors ${theme === 'light' ? 'text-slate-700 group-hover:text-slate-900' : 'text-slate-300 group-hover:text-slate-100'}`}>Shuffle answer choices in MCQs</span>
                </label>
              </div>
      ...
      {showAddQuestionsModal && (
        <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto backdrop-blur-sm ${theme === 'light' ? 'bg-slate-900/40' : 'bg-black/70'}`}>
          <div className={`relative w-full max-w-4xl my-8 rounded-2xl border shadow-2xl max-h-[90vh] overflow-y-auto ${theme === 'light' ? 'border-slate-300 bg-white' : 'border-slate-700/60 bg-slate-900 shadow-black/60'}`}>
            {/* Sticky header */}
            <div className={`sticky top-0 z-10 flex items-center justify-between px-8 py-5 border-b backdrop-blur-sm ${theme === 'light' ? 'border-slate-200 bg-white/95' : 'border-slate-800 bg-slate-900/95'}`}>
              <div>
                <h2 className={`text-lg font-bold ${theme === 'light' ? 'text-slate-900' : 'text-slate-100'}`}>Add Questions to Exam</h2>
            ...
            <div className="p-8">
              {questions.length === 0 ? (
                ...
                <>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-3">
                              <div className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-all ${selected ? 'bg-cyan-500 border-cyan-500' : theme === 'light' ? 'border-slate-400' : 'border-slate-600'}`}>
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex flex-wrap items-center gap-2 mb-2">
                                ...
                                {q.difficulty && (
                                  <span className={`text-[0.6rem] font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${theme === 'light' ? 'border-slate-300 bg-slate-100 text-slate-600' : 'border-slate-600 bg-slate-800 text-slate-400'}`}>
                                    {q.difficulty}
                                  </span>
                                )}
                              </div>
                              <p className={`text-sm leading-snug ${theme === 'light' ? 'text-slate-800' : 'text-slate-300'}`}>{q.content.question}</p>
                            </div>
                          </div>
                        </div>
                      );
                  ...
                  <div className={`flex gap-3 sticky bottom-0 pt-4 border-t -mx-8 px-8 pb-1 ${theme === 'light' ? 'bg-white border-slate-200' : 'bg-slate-900 border-slate-800'}`}>
                    <button onClick={() => { setShowAddQuestionsModal(false); setSelectedQuestions([]); }}
                      className={`flex-1 rounded-xl border py-2.5 text-sm font-semibold transition-all ${theme === 'light' ? 'border-slate-300 bg-slate-100 text-slate-700 hover:border-slate-400 hover:text-slate-900' : 'border-slate-700 bg-slate-800/60 text-slate-400 hover:border-slate-600 hover:text-slate-200'}`}>
                      Cancel
                    </button>
